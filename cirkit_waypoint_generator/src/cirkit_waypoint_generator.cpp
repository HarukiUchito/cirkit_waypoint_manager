...
public:
    ...
    {
        return sqrt(pow(new_pose.pose.position.x - last_pose_.pose.position.x, 2) + pow(new_pose.pose.position.y - last_pose_.pose.position.y, 2));
    }
    ...
    void processFeedback(const visualization_msgs::InteractiveMarkerFeedbackConstPtr &feedback)
    {
        ...
        {
            reach_threshold_markers_.markers[std::stoi(feedback->marker_name)].pose = feedback->pose;
...
int main(int argc, char **argv)
{
    ...
    desc.add_options()("help", "Print help message")("load", boost::program_options::value<std::string>(), "waypoint filename");
